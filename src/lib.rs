...
use ngx::{
    core::{Buffer, NGX_CONF_OK, Status},
    ffi::{
...
impl http::HttpModule for Module {
    fn module() -> &'static ngx_module_t {
        unsafe { &*ptr::addr_of!(ngx_http_prometheus_exporter_module) }
    }
    ...
    unsafe extern "C" fn postconfiguration(cf: *mut ngx_conf_t) -> ngx_int_t {
        ...
        if !handler.is_null() {
            ...
            unsafe {
                *handler = Some(prometheus_exporter_content_handler);
            }
        } else {
...
static mut NGX_HTTP_PROMETHEUS_EXPORTER_COMMANDS: [ngx_command_t; 2] = [
    ngx_command_t {
        ...
        set: Some(ngx_http_prometheus_exporter_commands_set_enable),
...
static NGX_HTTP_PROMETHEUS_EXPORTER_MODULE_CTX: ngx_http_module_t =
    ngx_http_module_t {
...
ngx::ngx_modules!(ngx_http_prometheus_exporter_module);

#[used]
#[allow(non_upper_case_globals)]
#[cfg_attr(not(feature = "export-modules"), unsafe(no_mangle))]
pub static mut ngx_http_prometheus_exporter_module: ngx_module_t =
    ngx_module_t {
        ctx: std::ptr::addr_of!(NGX_HTTP_PROMETHEUS_EXPORTER_MODULE_CTX) as _,
        commands: unsafe {
...
http_request_handler!(
    prometheus_exporter_content_handler,
    |request: &mut http::Request| {
        let co =
            Module::location_conf(request).expect("module config is none");
...
",
        ...
        // Buffer get's cleaned by nginx since it's created via pool
...
extern "C" fn ngx_http_prometheus_exporter_commands_set_enable(
    cf: *mut ngx_conf_t,
    _cmd: *mut ngx_command_t,
    conf: *mut c_void,
) -> *mut c_char {
    // SAFETY: this function is called with non-NULL cf and conf always
    ...
    if args.nelts == 2 {
        ...
        // SAFETY: We know elts has 2 elements
        let elt_val = unsafe { *elts.add(1) };
        let val = elt_val.to_str();
